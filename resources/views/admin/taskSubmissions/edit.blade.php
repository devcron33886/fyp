...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.task-submissions.update", [$taskSubmission->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('task') ? 'is-invalid' : '' }}" name="task_id" id="task_id" required>
                    @foreach($tasks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('task_id') ? old('task_id') : $taskSubmission->task->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('task'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $taskSubmission->url) }}" required>
                @if($errors->has('url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('url') }}
                    </div>
            ...
            <div class="form-group">
                <label class="required" for="submitted_by_id">{{ trans('cruds.taskSubmission.fields.submitted_by') }}</label>
                <select class="form-control select2 {{ $errors->has('submitted_by') ? 'is-invalid' : '' }}" name="submitted_by_id" id="submitted_by_id" required>
                    @foreach($submitted_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('submitted_by_id') ? old('submitted_by_id') : $taskSubmission->submitted_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
